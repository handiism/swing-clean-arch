...
public class User implements repository.User {
    ...
    public entity.User Store(entity.User user) {
        try {
            pstmt = database.connection().prepareStatement("INSERT INTO " +
                    "user (username,password) VALUES (?, ?)", new String[]{
                    "id", "username", "password"});
            pstmt.setString(1, user.getUsername());
            pstmt.setString(2, user.getPassword());

            pstmt.executeUpdate();
            Log.println(Level.INFO, String.format("INSERT INTO user " +
    ...
    public entity.User Fetch(String username) {
        ...
        try {
            pstmt = database.connection().prepareStatement("SELECT * FROM " +
                    "user WHERE username = ?");
            pstmt.setString(1, username);

            ResultSet keys = pstmt.executeQuery();
            Log.println(Level.INFO, String.format("SELECT * FROM user WHERE " +
                    "username = '%s'", username));
